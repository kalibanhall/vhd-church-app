 ...
 */
router.post('/appointments/respond', auth_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.get('/availability', auth_1.authenticate, async (req, res) => {
    try {
        const pastorId = req.user?.id;
        if (!pastorId) {
            return res.status(401).json({
 ...
 */
router.post('/availability', auth_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.delete('/availability/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const pastorId = req.user?.id;
 ...
 */
router.get('/unavailability', auth_1.authenticate, async (req, res) => {
    try {
        const pastorId = req.user?.id;
        if (!pastorId) {
            return res.status(401).json({
 ...
 */
router.post('/unavailability', auth_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.delete('/unavailability/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const pastorId = req.user?.id;
        const { error } = await supabase
