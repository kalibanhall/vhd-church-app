...
router.get('/', async (req, res) => {
    try {
        ...
        ${genre ? (0, database_1.default) `AND genre = ${genre}` : (0, database_1.default) ``}
        ${artist ? (0, database_1.default) `AND artist_id = ${artist}` : (0, database_1.default) ``}
        ${search ? (0, database_1.default) `AND (title ILIKE ${'%' + search + '%'} OR artist_name ILIKE ${'%' + search + '%'})` : (0, database_1.default) ``}
...
router.get('/artists', async (req, res) => {
    try {
        try {
            const artists = await (0, database_1.default) `
        SELECT 
...
// GET - Playlists
router.get('/playlists', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        try {
            const playlists = await (0, database_1.default) `
        SELECT 
          p.*,
